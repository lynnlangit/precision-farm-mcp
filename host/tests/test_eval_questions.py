...
async def test_q2_east_80_is_a_genuine_one_season_outlier():
    async with MCPFleet(["report-export"]) as fleet:
        r = await fleet.call("report-export", "bad_field_or_bad_year", field_name="East 80")
...
async def test_q3_west_120_is_a_normal_unremarkable_field():
    async with MCPFleet(["report-export"]) as fleet:
        r = await fleet.call("report-export", "bad_field_or_bad_year", field_name="West 120")
...
async def test_q5_ranking_over_explicit_named_years():
    async with MCPFleet(["report-export"]) as fleet:
        r = await fleet.call("report-export", "which_fields_made_money", seasons=[2019, 2020])
